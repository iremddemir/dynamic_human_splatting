...
exp_name = f"2_GENERATING_CANON_HUMAN_W_OFFSET"
...
for step in tqdm(range(iterations), desc="Training", total=iterations):
    ...
    for i in range(0, len(image_ids), 2):
        ...
        eq = (
            (smpl_locs[:, None, :] == mask_pixels_locs[None, :, :])
            .all(dim=2)
            .any(dim=1)
        )  # shape (V, M, 2)
        ...
        pts = smpl_locs[~eq]  # (M,2)
        if pts.numel() > 0:
            xs = pts[:, 0]  # x coords
            ys = pts[:, 1]  # y coords
            rgb[:, ys, xs] = 0.5
        torchvision.utils.save_image(
            rgb, f"./_irem/{folder_name}/{image_idx}_smpl_vertices_eq_masked.png"
        )

        rgb = preprocessed_rgb.squeeze(0).clone()  # (3,H,W)
        pts = mask_pixels_locs  # (M,2)
        if pts.numel() > 0:
            xs = pts[:, 0]  # x coords
            ys = pts[:, 1]  # y coords
            rgb[:, ys, xs] = 0.5
        torchvision.utils.save_image(
            rgb, f"./_irem/{folder_name}/{image_idx}_overlay_mask.png"
        )
        ###
        ...
        torchvision.utils.save_image(
            image, f"./_irem/{folder_name}/{image_idx}_human_gaussians_masked_black.png"
        )
        ...
        torchvision.utils.save_image(
            image, f"./_irem/{folder_name}/{image_idx}_human_gaussians_masked_white.png"
        )
        ...
        #############
        """ 
            1. find the closest smpl vertex to each 'human_gaussians_vis' with 2D distance vector
            2. 
        """
        # Step 1: Filter only visible SMPL vertices
        visible_indices = smpl_visibility.nonzero(as_tuple=True)[
        ...
        set_gaussian_model_features(gaussians, unmatched_human_gaussian_features)
        render_pkg = render_gaussians(gaussians, camera_list, bg="white")
        image, viewspace_point_tensor, visibility_filter, radii = (
            render_pkg["render"],
            render_pkg["viewspace_points"],
        ...
        torchvision.utils.save_image(
            image, f"./_irem/{folder_name}/{image_idx}_unmatched_human_gaussians.png"
        )
        ...
        rgb = preprocessed_rgb.squeeze(0).clone()  # (3,H,W)
        pts = unmatched_human_gaussian_locs  # (M,2)
        if pts.numel() > 0:
            xs = pts[:, 0]  # x coords
            ys = pts[:, 1]  # y coords
            rgb[:, ys, xs] = 0.5
        torchvision.utils.save_image(
            rgb, f"./_irem/{folder_name}/{image_idx}_unmatched_gaussians.png"
        )
        ...
        if (
            ...
            print("⚠️ ROTATION IS NOT CORRECT!")
            exit()

        """
            add those offset_canonical to valid_smpl_indices.
        """
        offsets_canonical = torch.matmul(
            offsets / smpl_scale, smpl_R.T
        )  # no need for transl, because the offset is relative

        raw_norm = unmatched_human_gaussian_features["rotation"].norm(
            dim=-1, keepdim=True
        )  # (..., 1)
        normed_rots = torch.nn.functional.normalize(
            unmatched_human_gaussian_features["rotation"], dim=-1
        )  # (..., 4)
        gs_rotmat = quaternion_to_matrix(normed_rots)  # (..., 3, 3)
        deformed_gs_rotmat = (
            lbs_T[valid_smpl_indices, :3, :3] @ gs_rotmat
        )  # (..., 3, 3)
        ...
        offsets_gaussians_canonical = {
            "xyz": deformed_smpl_at_canonical[0][valid_smpl_indices]
            + offsets_canonical,
        ...
        rr.log(
            f"world/canonical_human_with_offset",
            rr.Points3D(
                positions=offsets_gaussians_canonical["xyz"].detach().cpu().numpy()
            ),
        )

        ###### transformerd offsets to VGGT world

        offsets_gaussians_canonical_transformed = {
        ...
        set_gaussian_model_features(gaussians, offsets_gaussians_canonical_transformed)
        render_pkg = render_gaussians(gaussians, camera_list, bg="white")
        image, viewspace_point_tensor, visibility_filter, radii = (
            render_pkg["render"],
            render_pkg["viewspace_points"],
        ...
        torchvision.utils.save_image(
            image, f"./_irem/{folder_name}/{image_idx}_canonical_offset_white.png"
        )
        ...
        new_features = {
            ...
            "scaling": canonical_human_scales + smpl_scale.log(),
            "rotation": canonical_human_rotation,
        }
        # set_gaussian_model_features(gaussians, new_features)
        # render_pkg = render_gaussians(gaussians, camera_list)
        # image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]
        # torchvision.utils.save_image(image, f"./_irem/{folder_name}/{image_idx}_canonical_human_black.png")

        ###########
        merged_features = {
            "xyz": torch.cat(
                [new_features["xyz"], offsets_gaussians_canonical_transformed["xyz"]],
                dim=0,
            ),
            "features_dc": torch.cat(
                [
                    new_features["features_dc"],
                    offsets_gaussians_canonical_transformed["features_dc"],
                ],
                dim=0,
            ),
            "features_rest": torch.cat(
                [
                    ...
                    offsets_gaussians_canonical_transformed["features_rest"],
                ],
                dim=0,
            ),
            "opacity": torch.cat(
                [
                    ...
                    offsets_gaussians_canonical_transformed["opacity"],
                ],
                dim=0,
            ),
            "scaling": torch.cat(
                [
                    ...
                    offsets_gaussians_canonical_transformed["scaling"],
                ],
                dim=0,
            ),
            "rotation": torch.cat(
                [
                    ...
                    offsets_gaussians_canonical_transformed["rotation"],
                ],
                dim=0,
            ),
        }
        ...
        torchvision.utils.save_image(
            image,
            f"./_irem/{folder_name}/{image_idx}_canonical_human_w_offset_black.png",
        )
        ...
        torchvision.utils.save_image(
            image,
            f"./_irem/{folder_name}/{image_idx}_canonical_human_w_offset_white.png",
        )
        ###########

        """
        merged human oluşturma
        """
        ...
        torchvision.utils.save_image(
            image, f"./_irem/{folder_name}/{image_idx}_matched_gaussians_black.png"
        )
        ...
        torchvision.utils.save_image(
            image, f"./_irem/{folder_name}/{image_idx}_matched_gaussians_white.png"
        )
        #######
        ...
        set_gaussian_model_features(gaussians, merged_features)
        render_pkg = render_gaussians(gaussians, camera_list, bg="green")
        image, viewspace_point_tensor, visibility_filter, radii = (
            render_pkg["render"],
            render_pkg["viewspace_points"],
            render_pkg["visibility_filter"],
        ...
        torchvision.utils.save_image(
            image,
            f"./_irem/{folder_name}/{image_idx}_canon2pose_human_with_scene_green.png",
        )
        #######

        #######

        raw_norm = offsets_gaussians_canonical["rotation"].norm(
            dim=-1, keepdim=True
        )  # (..., 1)
        normed_rots = torch.nn.functional.normalize(
            offsets_gaussians_canonical["rotation"], dim=-1
        )  # (..., 4)
        gs_rotmat = quaternion_to_matrix(normed_rots)  # (..., 3, 3)
        deformed_gs_rotmat = (
            lbs_T[valid_smpl_indices, :3, :3] @ gs_rotmat
        ...
        offsets_vggt = torch.matmul(offsets_canonical, smpl_R) * smpl_scale
        ...
        set_gaussian_model_features(gaussians, offsets_gaussians_canonical_to_posed)
        render_pkg = render_gaussians(gaussians, camera_list, bg="white")
        image, viewspace_point_tensor, visibility_filter, radii = (
            render_pkg["render"],
            render_pkg["viewspace_points"],
        ...
        torchvision.utils.save_image(
            image, f"./_irem/{folder_name}/{image_idx}_canon2pose_only_offsets.png"
        )

        # # # merged_features = {
        # # #     "xyz":           torch.cat([features_scene["xyz"], new_features["xyz"], unmatched_human_gaussian_features["xyz"]], dim=0),
        # # #     "features_dc":   torch.cat([features_scene["features_dc"], new_features["features_dc"], unmatched_human_gaussian_features["features_dc"]], dim=0),
        # # #     "features_rest": torch.cat([features_scene["features_rest"], new_features["features_rest"], unmatched_human_gaussian_features["features_rest"]], dim=0),
        # # #     "opacity":       torch.cat([features_scene["opacity"], new_features["opacity"], unmatched_human_gaussian_features["opacity"]], dim=0),
        # # #     "scaling":       torch.cat([features_scene["scaling"], new_features["scaling"], unmatched_human_gaussian_features["scaling"]], dim=0),
        # # #     "rotation":      torch.cat([features_scene["rotation"], new_features["rotation"], unmatched_human_gaussian_features["rotation"]], dim=0)
        # # # }
        # # # set_gaussian_model_features(gaussians, merged_features)
        # # # render_pkg = render_gaussians(gaussians, camera_list, bg="white")
        # # # image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]
        # # # torchvision.utils.save_image(image, f"./_irem/{folder_name}/{image_idx}_canon2pose_human_with_scene_with_offsets.png")
        # # # #######
        ...
        torchvision.utils.save_image(
            image,
            f"./_irem/{folder_name}/{image_idx}_canon2pose_human_with_scene_with_offsets.png",
        )
        #######
