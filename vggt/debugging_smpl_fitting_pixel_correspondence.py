...
from instantsplat.scene.gaussian_model_without_act import GaussianModel_Without_Act
...
rr.init("pixel_correspondence", recording_id="v0.1")
...
#########################################################################################################################
from utils import get_deformed_gs_xyz, get_deformed_gs_xyz_with_faces
from utils import preprocess_masks, preprocess_masks_and_transforms, transform_indices
from utils import get_canonical_from_posed_xyz

betas = torch.tensor(
    [-0.2437, 0.3340, 0.2501, 0.9559, -0.2041, 0.2652, 0.0610, 0.0701, 0.2794, -0.1130],
    device="cuda:0",
)
...
global_orient_list = torch.tensor(
    [
        [2.5507e00, 1.5618e-01, -1.7017e00],
        [2.5218e00, 1.9599e-01, -1.7007e00],
        [2.6574e00, 1.4839e-01, -1.5810e00],
        [2.6400e00, 7.7774e-02, -1.8512e00],
        [2.5756e00, 1.1379e-01, -1.8062e00],
        [2.4858e00, 1.9192e-01, -1.9071e00],
        [-2.0643e00, -2.0246e-01, 2.3941e00],
        [-1.9345e00, -8.6205e-02, 2.5449e00],
        [-1.5405e00, -9.7832e-02, 2.7556e00],
        [-1.7772e00, -8.0309e-02, 2.5723e00],
        [-2.0435e00, -1.8757e-01, 2.3943e00],
        [-2.0585e00, -2.2649e-01, 2.4143e00],
        [2.5392e00, 7.1876e-02, -1.8250e00],
        [2.5960e00, 8.0192e-02, -1.7725e00],
        [2.9626e00, 1.5595e-01, -1.0049e00],
        [3.5445e00, 1.1901e-02, -5.0003e-01],
        [2.7881e00, 2.7429e-02, -2.5197e-01],
        [2.8812e00, -1.8224e-02, -9.7921e-02],
        [3.3727e00, 2.2536e-02, 1.0087e-03],
        [2.9282e00, 8.0263e-02, -4.1888e-02],
        [2.7833e00, 8.3970e-02, -4.8340e-02],
        [3.0304e00, 8.5194e-02, -4.6215e-01],
        [2.2984e00, 1.5377e-02, -2.0393e00],
        [-1.6577e00, -2.3015e-01, 2.6511e00],
        [-1.0902e00, -1.5742e-01, 2.8249e00],
        [-1.7314e-01, -9.8048e-02, 3.0357e00],
        [1.7290e00, -1.4900e-01, 2.6024e00],
        [2.4632e00, -1.4639e-01, 1.9511e00],
        [2.7748e00, -9.2010e-02, 1.5359e00],
        [2.8781e00, -5.1537e-02, 1.4405e00],
        [2.7431e00, 1.3850e-03, 1.5807e00],
        [2.5524e00, -1.2430e-02, 1.8938e00],
        [2.7302e00, 2.3178e-02, 1.4501e00],
        [3.0075e00, -1.9699e-01, 1.1618e00],
        [2.7826e00, 1.8080e-02, 1.3743e00],
        [2.8112e00, 4.6875e-02, 1.3301e00],
        [2.8040e00, 8.8143e-02, 1.2950e00],
        [2.9620e00, 3.2971e-02, 7.5489e-01],
        [3.0971e00, 3.9638e-02, -7.2873e-01],
        [2.7261e00, 2.2681e-01, -1.2892e00],
        [2.6075e00, 1.8707e-01, -1.8225e00],
        [-2.1444e00, -2.3249e-01, 2.2018e00],
        [-5.5775e-01, -3.2646e-01, 3.0539e00],
        [-7.0283e-02, -2.3760e-01, 3.1661e00],
        [4.8928e-01, -6.7706e-02, 3.1473e00],
        [1.1932e00, 3.5819e-02, 2.8709e00],
        [2.2616e00, -2.0202e-01, 2.0055e00],
        [2.4745e00, -2.7918e-01, 1.7526e00],
        [3.0187e00, -1.3482e-01, 5.7206e-01],
        [3.0275e00, -1.1759e-01, -2.1951e-01],
        [2.8568e00, 3.2015e-02, -9.0155e-01],
        [2.8279e00, 1.2257e-01, -8.7364e-01],
        [2.8188e00, 1.5682e-01, -9.3382e-01],
        [2.9048e00, 1.6119e-01, -8.3946e-01],
        [2.9031e00, 1.4373e-01, -8.3072e-01],
        [2.9461e00, 1.2401e-01, -8.0343e-01],
        [2.9591e00, 1.2936e-01, -8.2766e-01],
        [2.9230e00, 1.3196e-01, -8.8014e-01],
        [2.8865e00, 1.4276e-01, -8.5508e-01],
        [2.7707e00, 1.5358e-01, -8.5133e-01],
        [2.8581e00, 1.2572e-01, -7.6502e-01],
        [2.8763e00, 8.7062e-02, -1.0667e00],
        [2.8333e00, 7.7667e-02, -1.1034e00],
        [2.9069e00, -8.5062e-03, -9.7691e-01],
        [2.6375e00, 8.1234e-02, -6.3163e-01],
        [2.7049e00, 1.8650e-01, -5.9456e-01],
        [3.1767e00, 1.3084e-01, -2.3982e-01],
        [3.1581e00, 1.4581e-01, -2.8433e-01],
        [3.1952e00, 6.2761e-02, -2.5122e-01],
        [3.1570e00, -3.8887e-02, -3.8189e-01],
        [2.9881e00, 2.3164e-01, -6.4205e-01],
        [3.1078e00, 1.9512e-01, -5.4446e-01],
        [3.1288e00, 1.8413e-01, -6.4886e-01],
        [3.1471e00, 1.4836e-01, -6.5759e-01],
        [2.9901e00, 6.3522e-02, -9.3687e-01],
        [2.8863e00, 2.3543e-01, -1.0751e00],
        [2.8008e00, 3.3032e-01, -1.3102e00],
        [2.7216e00, 3.3079e-01, -1.5426e00],
        [2.7564e00, 2.3114e-01, -1.5679e00],
        [2.6181e00, 2.9720e-01, -1.6509e00],
        [2.6221e00, 3.4254e-01, -1.5928e00],
        [2.7152e00, 2.7667e-01, -1.5903e00],
    ],
    device="cuda:0",
)
...
#     t,          # (3,) camera-to-world translation
...
#     # --- 1) build intrinsics from FOVs ---
#     fx = (width  / 2) / math.tan(fovx / 2)
#     fy = (height / 2) / math.tan(fovy / 2)
#     cx = (width  - 1) / 2
#     cy = (height - 1) / 2

#     # --- 2) world → camera coords ---
#     # invert your c2w: X_cam = Rᵀ (X_world - t)
#     X_cam = ( R.T @ (vertices - t[None]).T ).T   # (N,3)
...
#     # y-axis in images goes down, so if your camera coords are y-up you may want:
#     # vs = cy - (X_cam[:,1] / zs) * fy

#     x_pix = us.round().long().clamp(0, width  - 1)
#     y_pix = vs.round().long().clamp(0, height - 1)

#     # --- 4) z-buffer so each pixel only keeps its closest point ---
#     depth_buffer = torch.full((height, width), float('inf'), device=device)
#     idx_buffer   = torch.full((height, width),     -1,      dtype=torch.long, device=device)
...
#     # --- 5) invert map: per-point pixel + valid mask ---
#     pixel_coords = torch.full((N,2), -1, dtype=torch.long, device=device)
#     valid_mask   = torch.zeros(N,    dtype=torch.bool, device=device)
...
for step in tqdm(range(iterations), desc="Training", total=iterations):
    ...
    rr.log(
        ...
        rr.Points3D(positions=human_in_hugs_world.detach().cpu().numpy()),
    )
    ...
    gaussians = GaussianModel_Without_Act(0)
    gaussians.init_RT_seq({1.0: camera_list})

    features = {
        "xyz": human_in_hugs_world,
        "features_dc": torch.zeros(
            (human_in_hugs_world.detach().view(-1, 3, 1).shape[0], 1, 3),
            device=human_in_hugs_world.device,
        ),
        "features_rest": torch.zeros(
            (human_in_hugs_world.detach().view(-1, 3, 1).shape[0], 15, 3),
            device=human_in_hugs_world.device,
        ),
        "opacity": torch.zeros(
            (human_in_hugs_world.detach().view(-1, 3, 1).shape[0], 1),
            device=human_in_hugs_world.device,
        )
        + 1,
        "scaling": torch.zeros(
            (human_in_hugs_world.detach().view(-1, 3, 1).shape[0], 3),
            device=human_in_hugs_world.device,
        )
        + 0.05,
        "rotation": torch.zeros(
            (human_in_hugs_world.detach().view(-1, 3, 1).shape[0], 4),
            device=human_in_hugs_world.device,
        ),
    }
    ...
    if step % 100 == 0:
        torchvision.utils.save_image(
            image,
            f"./_irem/{folder_name}/render_step_{step}_cam_{i}_exp_{exp_name}.png",
        )

    # a, b = project_vertices_to_pixels(human_in_hugs_world, human_faces, R, t, data["fovx"], data["fovy"], data["image_height"], data["image_width"])
    a, b = project_vertices_to_pixels(
        human_in_hugs_world,
        human_faces,
        data["c2w"],
        data["fovx"],
    ...
    # a, b = project_vertices_to_pixels(human_in_hugs_world, data["full_proj_transform"], data["image_height"], data["image_width"])

    # # # # H = data["image_height"]
    # # # # W = data["image_width"]
    # # # # mask_img = torch.ones((1, H, W), device=human_in_hugs_world.device)
    # # # # valid_pts = a[b]          # shape (M,2)
    # # # # if valid_pts.numel() > 0:
    # # # #     xs = valid_pts[:, 0]
    # # # #     ys = valid_pts[:, 1]
    # # # #     # 3) Paint those pixels black:
    # # # #     mask_img[0, ys, xs] = 0
    # # # # mask_rgb = mask_img.repeat(3, 1, 1)   # shape (3,H,W)
    # # # # torchvision.utils.save_image(mask_rgb, "./_irem/proj_mask.png")

    # # # rgb = data["rgb"].to(human_in_hugs_world.device).clone()  # (3,H,W)
    # # # pts = a[b]               # (M,2)
    # # # if pts.numel() > 0:
    # # #     xs = pts[:, 0]       # x coords
    # # #     ys = pts[:, 1]       # y coords
    # # #     rgb[:, ys, xs] = 0
    # # # torchvision.utils.save_image(rgb, "./_irem/overlay_points.png")

    preprocessed_mask, transforms = preprocess_masks_and_transforms([data["mask"]])
    new_coords_list = transform_indices(a, transforms[0])

    torchvision.utils.save_image(
        preprocessed_mask.squeeze(0), "./_irem/processed_mak.png"
    )
    rgb = preprocessed_mask.squeeze(0).to(human_in_hugs_world.device).clone()  # (3,H,W)
    pts = new_coords_list[b]  # (M,2)
    if pts.numel() > 0:
        xs = pts[:, 0]  # x coords
        ys = pts[:, 1]  # y coords
        rgb[:, ys, xs] = 0.5
    torchvision.utils.save_image(rgb, "./_irem/processed_overlay_points.png")
    ...
    if step % 100 == 0:
        ...
        print(f"./_irem/{folder_name}/render_step_{step}_cam_{i}_exp_{exp_name}.png")
    ...
    # deformed_xyz, deformed_rot4d, deformed_scale = get_deformed_gs_xyz(None, None, features_human["scaling"], features_human["rotation"], betas, pose, global_orient, smpl_scale, smpl_transl)
    # deformed_xyz, deformed_rot4d = get_canonical_from_posed_xyz(features_human["xyz"], features_human["rotation"], betas, pose, global_orient, smpl_scale, smpl_transl)
    # deformed_scale = features_human["scaling"]
    smpl_vertices = get_deformed_gs_xyz(
        None,
        None,
        features_human["scaling"],
        features_human["rotation"],
        betas,
        pose,
        global_orient,
        smpl_scale,
    ...
    find_smpl_to_gaussian_correspondence(smpl_vertices, features_human["xyz"])
    ...
    rr.log(
        f"world/scene",
        rr.Points3D(positions=features_scene["xyz"].detach().cpu().numpy()),
    )
    rr.log(
    ...
    rr.log(
        f"world/deformed_human",
        rr.Points3D(positions=deformed_xyz.detach().cpu().numpy()),
    )
    ...
    features_smpl = {
        ...
        "scaling": deformed_scale,
        "rotation": deformed_rot4d,
    }

    gaussians_for_canon_human = GaussianModel_With_Act(0)
    gaussians_for_canon_human.init_RT_seq({1.0: camera_list})
    set_gaussian_model_features(gaussians_for_canon_human, features_smpl)
    ...
    if step % 100 == 0:
        torchvision.utils.save_image(
            image,
            f"./_irem/{folder_name}/canon_human_scene_render_step_{step}_cam_{i}_exp_{exp_name}.png",
        )

    find_smpl_to_gaussian_correspondence(features_smpl["xyz"], features_human["xyz"])
    ...
    tqdm.write(
        f"Step {step}, Cam {i}, L1: {loss.item():.6f}, SSIM: {ssim_loss.item():.6f}, , LPIPS: {lpips_loss.item():.6f}, L1-Human: {l1_human.item():.6f}, SSIM-Human: {ssim_human.item():.6f}, LPIPS-Human: {lpips_human.item():.6f}, Grad-Norm: {total_norm:.3f}"
    )
