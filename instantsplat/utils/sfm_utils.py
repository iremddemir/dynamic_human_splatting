...
from instantsplat.dust3r.utils.image import _resize_pil_image
from instantsplat.dust3r.utils.device import to_numpy
from instantsplat.scene.colmap_loader import (
...
def save_intrinsics(sparse_path, focals_x,focals_y, org_imgs_shape, imgs_shape, save_focals=False):
    ...
    cameras = {}
    for i, (focal_x,focal_y) in enumerate(zip(focals_x,focals_y), start=1):  # Start enumeration from 1
        cameras[i] = Camera(
            id=i,
            model="PINHOLE",
            width=org_width,
            height=org_height,
            params=[focal_x*scale_factor_x, focal_y*scale_factor_y, org_width/2, org_height/2]
        )    
    print(f' - scaling focal: ({focal_x}, {focal_y}) --> ({focal_x*scale_factor_x}, {focal_y*scale_factor_y})' )
    write_cameras_binary(cameras, cameras_bin_file)
    write_cameras_text(cameras, cameras_txt_file)
    if save_focals:
        np.save(sparse_path / 'non_scaled_focals_x.npy', focals_x)
        np.save(sparse_path / 'non_scaled_focals_y.npy', focals_y)
