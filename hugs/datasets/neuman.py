...
def get_data_splits_ordered(scene):
    scene_length = len(scene.captures)
    num_val = scene_length // 5
    train_cutoff = scene_length - num_val
    train_list = list(range(train_cutoff))
    # Remaining frames for val and test
    val_test_list = list(range(train_cutoff, scene_length))
    half = len(val_test_list) // 2
    test_list = val_test_list[:half]
    val_list = val_test_list[half:]
    print(f"Train: {len(train_list)}, Val: {len(val_list)}, Test: {len(test_list)}")
    print(f"idxes train: {train_list}")
    print(f"idxes val: {val_list}")
    print(f"idxes test: {test_list}")
    assert len(train_list) > 0
    assert len(test_list) > 0
    assert len(val_list) > 0    
    return train_list, val_list, test_list
...
class NeumanDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if split == 'anim':
            ...
            self.train_split, _, self.val_split = get_data_splits_ordered(scene)
        else:
