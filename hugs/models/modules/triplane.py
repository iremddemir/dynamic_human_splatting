...
class TriPlane(nn.Module):
    ...
    def forward(self, x):
        ...
        x += 0.07217
        assert x.max() <= 1 + EPS and x.min() >= -EPS, (
            f"x must be in [0, 1], got {x.min()} and {x.max()}"
        )
        x = x * 2 - 1
        shape = x.shape
        ...
        feat_xy = F.grid_sample(self.plane_xy, coords[..., [0, 1]], align_corners=True)[
            0, :, :, 0
        ].transpose(0, 1)
        feat_xz = F.grid_sample(self.plane_xz, coords[..., [0, 2]], align_corners=True)[
            0, :, :, 0
        ].transpose(0, 1)
        feat_yz = F.grid_sample(self.plane_yz, coords[..., [1, 2]], align_corners=True)[
            0, :, :, 0
        ].transpose(0, 1)
